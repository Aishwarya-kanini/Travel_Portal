...
function AddPackage() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
        const response = await axios.post('https://localhost:7125/api/Agency', formData, {
            headers: {
  ...
  return (
    <Box>
      ...
      <Dialog open={showSuccessPopup} onClose={handleClosePopup}>
        <DialogTitle>Package Added Successfully</DialogTitle>
        <DialogContent>
          Your package has been added successfully. To check, go to "My Packages" page.
        </DialogContent>
        <DialogActions>
          ...
          <Button
            onClick={() => {
              ...
              window.location.href = '/PackageCard';
            }}
            color="primary"
          >
            Go to My Packages
