...
function PCard() {
  ...
  useEffect(() => {
    ...
    const fetchPhotos = async () => {
      try {
        const response = await axios.get('https://localhost:7111/api/Gallery');
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', justifyContent: 'center', marginTop: '55px' }}>
      {photos.map((photo) => (
        <Card key={photo.photo_Id} sx={{ maxWidth: 345, borderRadius: '10px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }}>
          <CardHeader
            avatar={
          ...
          <CardMedia
            component="img"
            height="194"
            image={`https://localhost:7111/uploads/images/${photo.tourGallery}`}
            alt="Paella dish"
            sx={{ objectFit: 'cover' }}
          />
          <CardContent>
